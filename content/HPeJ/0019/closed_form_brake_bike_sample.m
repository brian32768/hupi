%% closed_form_brake_bike_sample.m
...
% Plotting
subplot(2,2,1)
plot(t2b,rt*v2b, tfb,rt*vfb, trb,rt*vrb), grid
xlabel('Time, sec'), ylabel('Velocity, km/hr')
...
subplot(2,2,3)
plot(t2b,-a2b, tfb,-afb, trb,-arb), grid
xlabel('Time, sec'), ylabel('Deceleration, m/s^2')
subplot(2,2,4)
plot(t2b,-a2b/(g*mu), tfb,-afb/(g*mu), trb,-arb/(g*mu)), grid
xlabel('Time, sec'), ylabel('Braking Efficiency, \eta')